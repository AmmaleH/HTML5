...
```javascript
// 节点对象的三个属性： nodeType （节点类型）、nodeName（节点名称）、nodeValue（节点值）
// nodeType == 1：元素节点、2：属性节点、3：文本节点（包括文字、空格、换行）

// 节点层级
// 1.父级节点 Node.parentNode
// 2.子级节点 node.childNodes （得到集合，一般不使用，会获得文本节点）
//    子节点 node.children （获取所有的子元素节点）
// 3.node.firstChild  / node.lastChild   第一个 / 最后一个子节点
// 4.node.firstElementChild  / node.lastElementChild   第一个 / 最后一个子节点
// 5.node.nextSibling / previousSibling  下一个 / 上一个兄弟节点
// 6.node.nextElementSibling / previousElementSibling  下一个 / 上一个子兄弟元素节点

document.createElement('tagName')  // 动态创建元素节点
node.appendChild(child)    // 添加子节点
node.insertBefore(child, '指定元素')    // 插入节点
node.removeChild(child)     // 删除父节点内的指定child子节点
node.cloneNode()     // () 内为空或 false，则不复制内容， true则为深拷贝

// element.innerHTML 与 document.createElement() 区别
// createElement 效率更高，是以数组形式拼接，而 innerHTML 是拼接字符串。

e.preventDefault()  // 阻止事件默认行为
e.stopPropagation()  // 阻止时间冒泡
addEventListener false是冒泡（默认）
```
...
#####             1.GUI 渲染线程和 JS 引擎线程互斥

​				因为 JavaScript 可以操纵 DOM，如果在修改元素的同时渲染界面（即 JS 线程和 UI 线程同时运行），会导致渲染线程前后获得的元素数据可能不一致。为了防止渲染出现不可预期的结果，浏览器设置 GUI 渲染线程和 JS引擎为互斥的关系，当 JS 疫情执行时 GUI 线程会被挂起，GUI 的更新会被保存在在一个队列中等到 JS 引擎线程空闲时立即被执行。

##### 			2.JS 阻塞页面加载

​				根据上面互斥的关系，JS 如果执行时间过长会阻塞页面。当 JS 引擎在进行巨量的计算时，此时就算 GUI 有更新，也会被保存在队列中，等待 JS 引擎空闲后才执行。所以尽量避免 JS 执行时间过长，这样会造成页面的渲染不连贯，导致页面渲染加载阻塞，影响用户体验。

##### 			3.WebWorker

​				解决上面 JS 阻塞页面加载的问题。JS 引擎是单线程的，JS 执行时间过长会阻塞页面，WebWorker 用来解决 cpu 密集型计算。Worker 相当于浏览器给 JS 引擎开的外挂，在计算完结果后，将结果通信给主线程即可。

- 创建 Worker 时，JS 引擎向浏览器申请开一个子线程（子线程是浏览器开的，完全受主线程控制，而且不能操作DOM）；
- JS 引擎线程与 worker 线程间通过特定的方式通信（postMessage API，需要通过序列化对象来与线程交互特定的数据）。

​            4.WebWorker和SharedWorker

​				本质上是线程和进程的区别，WebWoker 只是渲染进程下的一个线程，SharedWorker 是由独立的进程管理。

- WebWorker 只属于某一个页面，不会和其他页面的渲染进程（浏览器内核进程）共享。Chrome浏览器在渲染进程中（每一个标签页就是一个渲染进程）创建一个新的线程来运行 Worker 中的 JS 程序；
- SharedWorker 是浏览器所有页面共享的，不能采用和 Worker 同样的方式实现，因为它不属于渲染进程，可以被多个渲染进程共享使用。Chrome浏览器为 SharedWorker 单独创建一个进程来运行 JS 程序，在浏览器中每个相同的 JS 只存在一个 SharedWorker 进程，不管它被创建过多少次。

####     **4.Browser 进程和渲染进程的通信过程**
...
​		前期工作：浏览器中输入 url，浏览器主进程接管，开一个下载线程。之后进行 http 请求，成功建立连接获得响应之后，将响应中的内容通过 RendererHost 接口转交给渲染进程。之后开始浏览器的渲染。

​		浏览器渲染的过程主要包括以下 5~6 步：

1. 浏览器将 HTML 文档解析成 DOM 树。
2. 解析 CSS，构成层叠样式表模型 CSSOM (CSS Object Model)。
3. 将 DOM 树和 CSSOM 合并为渲染树(`rendering tree`)，代表所有将被渲染的对象。
4. 布局渲染树（Layout/reflow），负责个元素尺寸、位置的计算。渲染树的每个元素包含的内容都是计算过的，它被称之为布局`layout`。浏览器使用一种流式处理的方法，只需要一次绘制操作就可以布局所有的元素。
5. 绘制渲染树（paint），绘制页面的像素信息（颜色等）。将渲染树的各个节点绘制到屏幕上，这一步被称为绘制`painting`。
6. 浏览器将各层的信息发送给 GPU，GPU 会将各层合成（composite），并进行显示。

####    6.CSS 加载是否阻塞 DOM 树渲染

​		不会。CSS 是由单独的下载线程异步下载的。CSS 加载不会阻塞 DOM 树解析（异步加载时 DOM 照常构建）；但是 CSS 加载会阻塞渲染树的渲染（渲染时需要等 CSS 加载完毕，因为渲染树需要 CSS 信息）。

#### 	7.普通图层 / 复合图层、硬件加速

​		浏览器渲染的图层一般包括两大类：普通图层和复合图层。

​		普通文档流内可以理解为一个复合图层（==默认复合层==，里面不管添加多少元素，其实都是在一个复合图层中）。在position 布局中的 absolute、fixed，虽然会脱离文档流，但仍然属于默认复合层。GPU 中，各个复合图层是单独绘制的，它们之间互不影响。可以通过硬件加速的方式，声明一个新的复合图层，它会单独分配资源。这样一来，它会脱离普通文档流，不管这个复合图层中怎么变化，也不会影响默认复合层中的重绘回流。

​		==复合图层的作用==：一般一个元素开启硬件加速后会变成复合图层，可以独立于普通文档流中，改动之后可以避免整个页面重绘，提升性能。但是大量使用复合图层，会造成资源消耗过度，导致页面变得卡顿。

##### 		**如何变成复合图层（硬件加速）**

​			将该元素变成一个复合图层，就是硬件加速技术。作用是提前告诉浏览器要变化，这样浏览器会开始做一些优化工作，最好用完后就释放。

- 最常用的方式：translate3d、translateZ；
- opacity属性 / transition 过渡动画（需要动画执行的过程中才会创建合成层，动画没有开始或者结束后元素还会回到之前的状态）；
- will-change 属性（比较少用），一般配合 opacity 和 translate 使用。（而且经过测试，除了上述可以引发硬件加速的属性外，其他属性并不会变成复合层）。

#####        硬件加速尽量使用 index

​			使用硬件加速的时候，尽可能地使用 index，防止浏览器默认给后续的元素创建复合层渲染。

​			具体的原理：webkit CSS3中，如果一个元素添加了硬件加速，并且 index 层级比较低，那么这个元素的后面其他元素（也就是层级比这个元素高的，或者相同的，并且 relative 或 absolute 属性相同的），会默认变为复合层渲染，如果处理不当会极大地影响性能。简单来说，可以认为是一个隐式合成的概念：如果 a 是一个复合图层，而且 b 在 a 上面，那么 b 也会被隐式转换成一个复合图层。

##### 		**absolute 和硬件加速的区别**

- absolute 虽然可以脱离普通文档流，但是无法脱离默认复合层。就算 absolute 中信息改变，也不会改变普通文档流中的渲染树。最终浏览器进行绘制时，是整个复合层绘制的，所以 absolute 中信息的改变，仍然会影响整个复合层的绘制。浏览器会重绘它，如果复合层中内容很多，absolute 带来的绘制信息变化过大，资源消耗是非常严重的。
- 硬件加速直接是另开一个复合图层了。所以他的信息改变不会影响默认复合层，内部的话肯定会影响属于自己的复合层。它仅仅是引发最后的合成（输出视图）。
...
​	跨域，是浏览器的一种保护机制，是为了保护站点的安全，跨域本身是不存在的，为了安全，浏览器针对XMLHTTPRequest 和 fetch 这种脚本发起的跨域请求做了一定的跨域限制，而对于浏览器自身的如 img 的 src、script 的 js 资源、form 表单的提交，没有做跨域的限制。==它是由浏览器的同源策略造成的，是浏览器对Javascript施加的安全限制==。**同源**：协议、域名、端口号三者皆相同。防止浏览器端恶意的抓取和篡改数据。

​	**1.为什么区分简单请求和非简单请求**

​		因为浏览器认为简单请求是安全的，所以可以通过跨域限制到达服务器，也就没必要对简单请求做预检请求（人家本来就可以通过跨域你预检个什么）
...
​		**CORS 请求过程**：首先如果是简单请求则在请求头直接添加orgin字段，对于非简单请求需要一次预检请求options，需要origin字段，当服务端通过预检请求会在响应头携带Allow-controll-Access-Origin字段(为*或者是origin的值)，之后的请求就像简单请求一样，携带origin字段。
...
​	Vue 只会对新旧节点中 父节点是相同节点 的 那一层子节点 进行比较。遵循 同层级比较，只有两个新旧节点是相同节点的时候，才会去比较他们各自的子节点。最大的根节点一开始可以直接比较，并不需要递归，虽然好像降低了一些复用性，也是为了避免过度优化，是一种很高效的 Diff 算法。
